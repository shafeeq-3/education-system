...
export default function Programs() {
  ...
  const [formData, setFormData] = useState({
    name: '', code: '', departmentId: '', duration: '', degreeType: '', description: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/programs/${editingId}`, formData);
  ...
  const handleEdit = async (id) => {
    try {
      ...
      setFormData({
        name: data.name || '', code: data.code || '', departmentId: data.department?._id || '',
        duration: data.duration || '', degreeType: data.degreeType || '', description: data.description || ''
      });
      setEditingId(id);
      setShowModal(true);
    } catch (err) {
  ...
  const resetForm = () => {
    setFormData({ name: '', code: '', departmentId: '', duration: '', degreeType: '', description: '' });
    setEditingId(null);
    setFieldErrors({});
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-teal-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">Manage academic programs and degrees</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {programs.map((program) => (
            <div key={program._id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all p-6">
              ...
              <div className="space-y-2 text-sm mb-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900">{program.department?.name || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                ...
                {program.degreeType && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900 capitalize">{program.degreeType}</span>
                  </div>
                )}
              </div>
              {program.description && (
                <p className="text-sm text-gray-600 mb-4 line-clamp-2 pb-4 border-b">{program.description}</p>
              )}
