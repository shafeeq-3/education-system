...
import { Plus, Edit2, Trash2, Search, Calendar, CheckCircle, AlertCircle, Loader2, X } from 'lucide-react';
...
export default function AcademicYears() {
  ...
  const [formData, setFormData] = useState({
    year: '', startDate: '', endDate: '', isCurrent: false
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/academic-years/${editingId}`, formData);
  ...
  const handleEdit = async (id) => {
    try {
      const response = await axios.get(`/academic-years/${id}`);
      ...
      error('Failed to fetch academic year details');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-teal-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">Manage academic year periods</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          ...
          <p className="text-gray-400 text-sm mt-2">Create your first academic year to get started</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {academicYears.map((year) => (
            <div key={year._id} className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">{year.year}</h3>
                    {year.isCurrent && (
                      <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800 font-medium inline-flex items-center gap-1">
              ...
              <div className="space-y-2 text-sm mb-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900">{new Date(year.startDate).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">End Date:</span>
                  <span className="font-medium text-gray-900">{new Date(year.endDate).toLocaleDateString()}</span>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Year (e.g., 2024-2025) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="year"
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label className="text-sm font-medium text-gray-700">Set as current academic year</label>
              </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t">
                ...
                >
                  {submitting ? (
                    ...
                    <>{editingId ? 'Update' : 'Create'} Academic Year</>
                  )}
                </button>
              </div>
            </form>
